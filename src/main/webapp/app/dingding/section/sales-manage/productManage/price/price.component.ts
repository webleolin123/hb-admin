...
export class PriceComponent implements OnInit,OnDestroy{
    ...
    constructor(
        ...
        this.priceService.resourceUrl=WORKFLOW_PRICE_MANAGE_API;
        this.priceService.resourceUrl_search=WORKFLOW_PRICE_MANAGE_SEARCH_API;
    }
    ngOnInit() {
    ...
    chartRequest(){
        ...
        this.searchDay=this.eventManager.subscribe('saveDayValue', response => {
            ...
            this.priceService.resourceUrl_getTrend= WORKFLOW_HOME_BUSINESS_DAY_TREND_API;//默认展示日的数据
            if(response.name=='saveDayValue'){
                this.priceService.getTrendData({
        ...
        this.searchMonth=this.eventManager.subscribe('saveMonthValue', response => {
            this.priceService.resourceUrl_getTrend= WORKFLOW_HOME_BUSINESS_MONTH_TREND_API;//展示月的数据
            console.log('response',response);
        ...
        this.searchYear=this.eventManager.subscribe('saveYearValue', response => {
            console.log('response',response);
            this.priceService.resourceUrl_getTrend= WORKFLOW_HOME_BUSINESS_YEAR_TREND_API;
    ...
    registerChangeInUsers() {
        // 编辑
        this.subs=this.eventManager.subscribe('priceListModification', (response) => {
        ...
        // 删除
        this.eventManager.subscribe('priceDeleteInfo', response => {
            if(response.content==200){
                this.loadAll();
                alert('删除成功');
            }
    ...
    loadAll() {
        this.router.navigate(['/dingding/section/price'], {
            queryParams: {
                page: this.page - 1,
                size: this.itemsPerPage,
            }
    ...
    // 添加新数据
    add() {
        this.priceService.approverCommonId = null;
        const activeModal = this.modalService.open(PriceAddComponent, {size: 'lg', container: 'nb-layout'});
    }

    // 编辑
    edit(id) {
        this.priceService.approverCommonId = id;
        const activeModal = this.modalService.open(PriceAddComponent, {size: 'lg', container: 'nb-layout'});
    }
    ...
    check(id) {
        this.priceService.approverCommonId = id;
        const activeModal = this.modalService.open(PriceDetailComponent, {size: 'lg', container: 'nb-layout'});
    }
    ...
    export() {
        ...
        //     this.startTime = this.priceService.startTime;
        //     this.endTime = this.priceService.endTime;
    ...
    exportDataTimeBetween() {
        // this.priceService.exportDataByTimeBetween({
