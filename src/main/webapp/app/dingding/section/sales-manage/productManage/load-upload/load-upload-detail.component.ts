...
export class LoadUploadDetailComponent implements OnInit {
    ...
    constructor(private shelveService: MyHttpService,
    ...
    load(){
        if (this.shelveService.approverCommonId) {
            this.shelveService.find(this.shelveService.approverCommonId).subscribe(
    ...
    private onSuccess(data) {
        ...
        if (this.shelve.approveInfoDTO.copyPeople.length != 0) {
            this.user=[];
            this.shelve.approveInfoDTO.copyPeople.forEach((myCopyTo) => {
                if (myCopyTo != null) {
    ...
    //     var iDays = Math.floor(Math.abs(Date.parse(startDate) - Date.parse(endDate)) / (24 * 3600 * 1000) + 1);
    //     this.companyshelve = iDays * this.shelve.pershelve;
    //     this.factoryshelve = iDays * this.shelve.pershelveOfSupport;
    //     return iDays// 相差天数
    // }
    //
    // countMonth(startDate, endDate) {
    //     var iMonth = Math.floor(Math.abs(Date.parse(startDate) - Date.parse(endDate)) / (30 * 24 * 3600 * 1000) + 1);
    //     this.companyshelve = iMonth * this.shelve.pershelve;
    //     this.factoryshelve = iMonth * this.shelve.pershelveOfSupport;
    //     return iMonth;
    // }
