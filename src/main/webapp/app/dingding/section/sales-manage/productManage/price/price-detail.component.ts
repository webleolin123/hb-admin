...
export class PriceDetailComponent implements OnInit {
    ...
    constructor(private priceService: MyHttpService,
    ...
    load(){
        if (this.priceService.approverCommonId) {
            this.priceService.find(this.priceService.approverCommonId).subscribe(
    ...
    private onSuccess(data) {
        ...
        if (this.price.approveInfoDTO.copyPeople.length != 0) {
            this.user=[];
            this.price.approveInfoDTO.copyPeople.forEach((myCopyTo) => {
                if (myCopyTo != null) {
    ...
    countDays(startDate, endDate) {
        ...
        this.companyPrice = iDays * this.price.perPrice;
        this.factoryPrice = iDays * this.price.perPriceOfSupport;
        return iDays// 相差天数
    }
    countMonth(startDate, endDate) {
        const iMonth = Math.floor(Math.abs(Date.parse(startDate) - Date.parse(endDate)) / (30 * 24 * 3600 * 1000) + 1);
        this.companyPrice = iMonth * this.price.perPrice;
        this.factoryPrice = iMonth * this.price.perPriceOfSupport;
        return iMonth;
    }
