...
export class GiftComponent implements OnInit,OnDestroy {
    ...
    constructor(
        ...
        this.giftService.resourceUrl=WORKFLOW_GIFTS_API;
        this.giftService.resourceUrl_search=WORKFLOW_GIFTS_SEARCH_API;
    }
    ngOnInit() {
    ...
    chartRequest(){
        ...
        this.searchDay=this.eventManager.subscribe('saveDayValue', response => {
            ...
            this.giftService.resourceUrl_getTrend= WORKFLOW_HOME_BUSINESS_DAY_TREND_API;//默认展示日的数据
            if(response.name=='saveDayValue'){
                this.giftService.getTrendData({
        ...
        this.searchMonth=this.eventManager.subscribe('saveMonthValue', response => {
            this.giftService.resourceUrl_getTrend= WORKFLOW_HOME_BUSINESS_MONTH_TREND_API;//展示月的数据
            console.log('response',response);
        ...
        this.searchYear=this.eventManager.subscribe('saveYearValue', response => {
            console.log('response',response);
            this.giftService.resourceUrl_getTrend= WORKFLOW_HOME_BUSINESS_YEAR_TREND_API;
    ...
    registerChangeInUsers() {
        this.subs=this.eventManager.subscribe('giftsListModification', (response) => {
    ...
    loadAll() {
        this.router.navigate(['/dingding/section/gift'], {
            queryParams: {
                page: this.page - 1,
                size: this.itemsPerPage,
            }
    ...
    // 添加新数据
    add() {
        this.giftService.approverCommonId = null;
        const activeModal = this.modalService.open(GiftAddComponent, {size: 'lg', container: 'nb-layout'});
    }

    // 编辑
    edit(id) {
        this.giftService.approverCommonId = id;
        const activeModal = this.modalService.open(GiftAddComponent, {size: 'lg', container: 'nb-layout'});
    }
    ...
    check(id) {
        this.giftService.approverCommonId = id;
        const activeModal = this.modalService.open(GiftDetailComponent, {size: 'lg', container: 'nb-layout'});
    }
    ...
    export() {
        ...
        //     this.startTime = this.giftService.startTime;
        //     this.endTime = this.giftService.endTime;
    ...
    exportDataTimeBetween() {
        // this.giftService.exportDataByTimeBetween({
