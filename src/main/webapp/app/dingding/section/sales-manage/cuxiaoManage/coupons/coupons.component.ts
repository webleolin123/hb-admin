...
export class CouponsComponent implements OnInit,OnDestroy{
    ...
    constructor(
        ...
        this.couponservice.resourceUrl=WORKFLOW_COUPONS_COUPON_API;
        this.couponservice.resourceUrl_search=WORKFLOW_COUPONS_SEARCH_API;
    }
    ngOnInit() {
    ...
    chartRequest(){
        ...
        this.searchDay=this.eventManager.subscribe('saveDayValue', response => {
            ...
            this.couponservice.resourceUrl_getTrend= WORKFLOW_HOME_BUSINESS_DAY_TREND_API;//默认展示日的数据
            if(response.name=='saveDayValue'){
                this.couponservice.getTrendData({
        ...
        this.searchMonth=this.eventManager.subscribe('saveMonthValue', response => {
            this.couponservice.resourceUrl_getTrend= WORKFLOW_HOME_BUSINESS_MONTH_TREND_API;//展示月的数据
            console.log('response',response);
        ...
        this.searchYear=this.eventManager.subscribe('saveYearValue', response => {
            console.log('response',response);
            this.couponservice.resourceUrl_getTrend= WORKFLOW_HOME_BUSINESS_YEAR_TREND_API;
    ...
    registerChangeInUsers() {
        this.subs=this.eventManager.subscribe('couponsListModification', (response) => {
    ...
    loadAll() {
        this.router.navigate(['/dingding/section/coupons'], {
            queryParams: {
                page: this.page - 1,
                size: this.itemsPerPage,
            }
    ...
    // 添加新数据
    add() {
        this.couponservice.approverCommonId = null;
        const activeModal = this.modalService.open(CouponsAddComponent, {size: 'lg', container: 'nb-layout'});
    }

    // 编辑
    edit(id) {
        this.couponservice.approverCommonId = id;
        const activeModal = this.modalService.open(CouponsAddComponent, {size: 'lg', container: 'nb-layout'});
    }
    ...
    check(id) {
        this.couponservice.approverCommonId = id;
        const activeModal = this.modalService.open(CouponsDetailComponent, {size: 'lg', container: 'nb-layout'});
    }
    ...
    export() {
        ...
        //     this.startTime = this.couponservice.startTime;
        //     this.endTime = this.couponservice.endTime;
    ...
    exportDataTimeBetween() {
        // this.couponservice.exportDataByTimeBetween({
